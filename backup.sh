...
clear
echo " VPS Data Backup By MS STORE SSH "
sleep 1
...
cd /root
zip -r $IP-$date-$domain-msstore.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-msstore.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-blueblue.zip)
id=(`echo $url | grep '^https' | cut -d'=' -f2`)
link="https://drive.google.com/u/4/uc?id=${id}&export=download"
clear
echo -e "\033[1;37mVPS Data Backup By MS STORE\033[0m
echo ""
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-msstore.zip
echo ""
